...
9. **Mehrparteien-Isolation (aus dem „authentik+HA RBAC"-Übergabepaket, §8 des Modells) — HÄRTETEST.** Der Paket-Use-Case ist echter Mehr-Wohneinheiten-Betrieb (getrennte Mieter EG/UG/DG, die sich NICHT sehen dürfen). Prüfe kritisch: (a) **Reicht Tesseras native-Gruppen-Permission-Modell für echte Tenant-Isolation**, oder sind die dokumentierten Leak-Pfade (Templates/Assist/Logbook/Recorder/WS-Event-Stream/History) ein hartes Sicherheits-Gap zwischen Mietern? Welche HA-nativen Permission-Prüfungen greifen wirklich bei `get_states`/`subscribe_events`/History/Logbook (belege gegen HA-Quelle, nicht Vermutung)? (b) Ist ein **`tenant_scope`/Label-Selektor** robuster als Area-Grants (Entity-ohne-Area-Problem)? Passt das zu Tesseras geplantem floor/label/category-Ausbau? (c) **Deny-Vorrang vs allow-only:** ist allow-only für „Bereich erlauben AUSSER Sensibles (Kamera/Schloss/Alarm)" ausreichend (via entity_overrides) oder ergonomisch unzureichend? (d) **OIDC-Integration:** liefert `hass-oidc-auth` (christiaangoossens) vs `auth_oidc v1.1.1` den rohen `groups`-Claim — was ist der belegte Stand für den by_group-Parallel-Track? Trenne dabei sauber: was ist bereits durch Tessera/`SECURITY.md` abgedeckt vs. was ist ein echter neuer Gap.
