...
## 9) Codex-Validierung (Ergebnis, 2026-07-01) — ACCEPT-Richtung / AUFLAGEN-Bau / NO-GO-sofort-by_group
Unabhängige Prüfung durch Codex' Agentensystem (2 Gegenpruef-Agenten, gegen `main`@`52bc92c`). **Schnittmenge = hohe Konfidenz** (Monitor nur Aggregat; by_group v1-inert braucht D12/Re-Gate; D9-Acks existieren ohne UI; by_user-Writer = Soak-Unblocker; Panel ist der richtige Ort; Wizard nachrangig+schmal). **Codex-Korrekturen an meinem Modell (übernommen):**
1. **`compute_enforce_plan` ist NOCH KEIN renderbarer PASS-Preflight.** Der Plan *gate't* (D9/Linter/Version/Store), aber die Erfolgs-Rückgabe enthält Groups/Bindings/Orphans/Block-Felder — **Owner-survives + Allow-only sind keine expliziten PASS-Felder**. Die Preflight-UI braucht ein **neues DTO** (sonst leitet sie ein falsches PASS ab). *(Meine Behauptung „nur rendern" war überhöht.)*
2. **Authentik parallel = Spezifikations-/Implementierungszweig, NICHT scharf.** NO-GO für sofortige De-Inertness. **Nur das Compiler-Flag zu drehen wäre gefährlich** — De-Inertness betrifft Compiler + Mode-Manager + Linter + Effective-View + Tests koordiniert. Mindest-Gates vor Aktivierung: Claim-Hook (redacted), Effective-Union `by_user ∪ valid_by_group` mit Herkunft, **Linter+Preflight prüfen dieselbe Union wie Apply**, leerer/kaputter Claim entfernt nie still Rollen (block/fail-safe), Owner-Survival-Tests, D12-Live-Beweis.
3. **Erster Bau-Schritt: `by_user`-Writer + minimale Effective-Read-Schicht** (nicht Deny-Log-first). Deny-Log wichtig, aber nicht der allererste Monitor-Soak-Blocker — vor dem enforce-Flip aber **entweder echte Deny-Evidence ODER ein klar als hypothetisch markierter Effective-Diff** (ein rein grün wirkender Preflight ohne Herkunft/Effekt ist psychologisch riskant).
4. **NEU — Doku↔Code-Divergenz (vor Authentik entscheiden):** User ohne wirksame Rolle — **Code fällt auf Default-Rolle zurück, `concept.md` fordert für bestimmte Fälle enforce-refusal.** Muss geklärt werden, bevor `by_group` scharf geht.
5. **`entity_overrides`** sind sicherheitsrelevant + backend-real, aber **UX-unsichtbar** — als bewusste Lücke führen.

**Sicherheits-AUFLAGEN für `tessera/membership/set` (Codex, verbindlich für den ersten PR):** admin-only (`require_admin`), **kein** direkter Native-Write; schema-validiert, unbekannte User/Rolle fail-closed; in enforce **zwingend derselbe zentrale `_compile_for_mode_safely`/Apply-Guard-Pfad** wie Matrix-Updates; redacted Audit (User-/Rollen-IDs ja, keine Claims/Secrets); Tests: unknown user/role, last-admin/owner-risk, owner/system-generated-Target-Rejection, kein Native-Write bei Block, recompile-failure-fail-safe.

**Reconcilte Bau-Reihenfolge:** (1) `by_user`-Writer (admin-only, recompile-safe, fail-closed) → (2) Effective-Access-Read/Preflight-DTO mit Herkunft + PASS/BLOCK → (3) Deny-Evidence (erst synthetisch, dann real+Retention) → (4) entity_overrides + Rollenpflege → (5) Authentik `by_group` nach D12/Re-Gate → (6) Wizard.

> **Isolations-Gate ENTSCHIEDEN:** Codex validierte das **UX-Modell**; die Mehrparteien-**Isolations**-Frage (§8.1) wurde separat quellcode-verifiziert beantwortet (`spike/reports/tessera-multitenant-isolation-feasibility-2026-07-01.md`): echte Mieter-Isolation auf **einer** Instanz = **NO-GO** (render_template/History/Automation-Kontext/Camera-Proxy/Admin-Bypass lecken lautlos). **Owner-Klärung 2026-07-01:** Pilzbuche 8 ist ein **Haushalt** (kein echter Mehrparteien-Fall) → **eine** Instanz + Tessera reicht, kein Split, Leak-Pfade tolerierbar. Tesseras Scope = **Ein-Haushalt-RBAC**. **Dieses UX-Modell wird für diesen Scope wie geplant gebaut** (T1 by_user-Writer → T2 Effective-Access → T3 Authentik).
