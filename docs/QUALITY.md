...
| Floor-Grants sind additive allow-only, fail-closed und laufen über guarded recompile | `test_floor_grant_compiles_to_entity_policy_leafs`, `test_overlapping_floor_and_area_grants_union_permissions`, `test_floor_grant_referencing_unknown_role_fails_closed`, `test_set_floor_grant_service_is_admin_only_and_persists`, `test_set_floor_grant_service_unknown_floor_fails_closed_without_save`, `test_set_floor_grant_service_unknown_role_preflights_all_entries`, `test_set_floor_grant_service_recompile_failure_fails_safe_to_monitor`, `test_set_floor_grant_service_blocked_enforce_plan_avoids_native_write` |
...
| `set_mode` ist admin-only, persistiert den gewählten Modus (nicht hartkodiert) und routet über den guarded `_compile_for_mode_safely`-Pfad (enter enforce → apply, leave enforce → restore, fail-safe delegiert); Schema akzeptiert nur off/monitor/enforce | `test_set_mode_service_is_admin_only_and_routes_through_guarded_path`, `test_set_mode_service_schema_restricts_to_known_modes` |
| Matrix-WS trennt Grants nach Quelle (Floor über den Floor der Area vs direkter Area-Grant), liefert `area_floor` (inkl. Floor-`level` + Registry-`order` für die Etagen-Gruppierung/Sortierung im Panel) + `entities_by_area`; eine Doppelvergabe (beide Quellen) ist an den unterschiedlichen Zellwerten erkennbar | `test_matrix_get_splits_floor_and_area_sources_with_entities` |
| Floor-Grant-Writer im Panel (`tessera/matrix/set_floor_grant`) ist admin-only, schema-aware (control⇒read), fail-closed bei unbekanntem Floor/Rolle, und re-appliet in enforce nativ über den guarded Mode-Handler (CXR-02) | `test_matrix_set_floor_grant_writes_and_control_implies_read`, `test_matrix_set_floor_grant_rejects_unknown_floor_and_role`, `test_matrix_set_floor_grant_in_enforce_reapplies_native` |
