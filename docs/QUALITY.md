...
| Matrix-WS trennt Grants nach Quelle (Floor über den Floor der Area vs direkter Area-Grant), liefert `area_floor` + `entities_by_area`; eine Doppelvergabe (beide Quellen) ist an den unterschiedlichen Zellwerten erkennbar | `test_matrix_get_splits_floor_and_area_sources_with_entities` |
