...
## Unsicherheiten
Verifiziert gegen **2025.1.4** (im venv), während Tesseras getestete Version **2026.6.4** ist (`SECURITY.md:72-76`) — die HA-Datei-/Zeilen-Anker sind damit **konservative Architektur-Evidenz, keine exakten 2026.6.4-Line-Proofs**. Die Architektur-Folgerung bleibt robust, weil Tesseras **eigene Doku dieselbe Scope-Grenze trägt** (`SECURITY.md:38-40`, `spec-enforce.md:10,31-32`, `concept.md:58,600`: view leakt, Tier-2-Hard-Isolation out-of-scope) — bestätigt durch Codex' Gesamtpaket-Review. Versions-Drift bleibt generell (HA ändert WS/Permissions/Templates häufig); Tesseras Invarianten-Halten unter allen Pfaden (Rebind/Race/Neustart) nicht erschöpfend auditiert; WS-Kommando-Vollständigkeit unbeweisbar (hunderte Kommandos, Add-ons registrieren eigene); Template-Service-Fähigkeit über alle custom_components nicht geprüft; Automation/Script-Kontext in Randfällen (Device-Actions, verschachtelte Ketten, Blueprints) fallweise.
